...
% THESIS TITLE MULTILANGUAGE STRING
...
\newtranslatorcommand{\myabstract}{%
   \import{sections/}{abstract_en.tex}%
}{%
    \import{sections/}{abstract_fi.tex}%
}
...
\begin{tabularx}{\linewidth}{|S{X}S{X}|S{X}|S{X}|S{X}|S{X}|}
    \Xhline{3\arrayrulewidth}
    % FIRST ROW FINNISH, FACULTY AND DEPARTMENT
    \multicolumn{3}{?>{\hsize=\dimexpr3\hsize-+3\tabcolsep\relax}S{X}|}{\textsf{Tiedekunta/Osasto  Fakultet/Sektion – Faculty}\newline \myfaculty} & %
    \multicolumn{3}{>{\hsize=\dimexpr3\hsize-+3\tabcolsep\relax}S{X}?}{\textsf{Laitos/Institution– Department}\newline \mydept} \TBstrut\\
    \hline
    % SECOND ROW, AUTHOR
    ...
    \hline    
    % SIXTH ROW, ABSTRACT
    \multicolumn{6}{?>{\hsize=\dimexpr6\hsize+6\tabcolsep\relax} S{X}?} {\textsf{Tiivistelmä/Referat – Abstract} \myabstract} \\ [500pt]
    \hline
    % SEVENTH ROW, KEYWORDS
    \multicolumn{6}{?>{\hsize=\dimexpr6\hsize-+6\tabcolsep\relax}S{X}?}{\textsf{Avainsanat – Nyckelord – Keywords}\newline \mykeywords} \TBstrut\\
    \hline
