...
Next in the survey data processing workflow the additional spatial data was added to \textit{postal}, the dataset with one row for each postal code area (figure~\ref{fig:gen_workflow}, section 4). Utilising \textit{records}, functions \code{sum}, \code{mean}, and \code{median} were used to produce answer count, and means and medians for parktime and walktime for all postal code area. Each postal code area also received seven columns to depict the share of \textit{YKR zones} classes in percentage. Using Urban Atlas 2012 data, the percentage of forest in each postal code area was calculated. 

In the finalising section \textit{records} was prepared to be used for analysis and visualisation in R (figure~\ref{fig:gen_workflow}, section 5).  The software library for plotting in R, ggplot2, prefers data inputted in long format. To study charasteristics of postal code areas in this research it meant adding repetitive data columns in \textit{records}, where values for Urban Atlas 2012 forest, YKR zone and subdivision remained unchanged for all rows in the same postal code area. For Urban Atlas 2012 forest, a custom Jenks breaks function (GitHub user Drewda, \textcolor{red}{add cite}) with five classes were utilised to find the applicable class for each postal code area. For YKR zones, the most common urban structure type in percentage was selected for each postal code area. In addition, \textit{records} was inserted with municipality subdivision (Finnish: "suuralue") information. This was achieved by gathering data from the web sites of the municipalities of Helsinki Capital Region (\textcolor{red}{lisää cite}). Each municipality broke the subdivisions down to city district level, from where it was possible to allot postal code areas to subdivisions. This was for the most part simplistic work, but in some cases the postal code areas and city districts did not match. One such area is south of Leppävaara in Espoo. \textcolor{red}{täydennä selitys leppävaarasta ja lisää subdivision-kartta}
