...
\begin{figure}[H]
    ...
    \caption{General workflow of survey data processing in Python and R.} 
...
\begin{hyphenrules}{nohyphenation}
    \begin{table}[H]
        \centering
        \caption[Thesis data]{Data utilised in the thesis. \textcolor{red}{delaa producer näistä}} 
        \label{tab:used_data}
        ...
        \begin{tabular}{ @{} >{\raggedright\arraybackslash}p{4cm} >{\raggedright\arraybackslash}p{4cm} >{\raggedright\arraybackslash}p{4.5cm} >{\raggedright\arraybackslash}p{3.5cm} >{\raggedleft\arraybackslash}p{3.5cm} @{} }
            \toprule
            Data & Description & Purpose in thesis & Producer & Citation \\
            \midrule
            Paavo -- Open data by postal code area & Helsinki Capital Region postal code areas & Thesis research area & Statistics Finland & \cite{StatisticsFinland2019a} \\
            Urban Atlas LCLU 2012 & Land use and land cover data in vector format for functional urban area of Helsinki & Source of forest data & European Environment Agency & \cite{EuropeanEnvironmentAgency2016} \\
            Zones of urban structure (Yhdyskuntarakenteen vyöhykkeet) 2017 & Delineation of urban areas based on the theory of urban fabrics & Source of data on spatial structure of urban areas & Finnish Environment Institute & \cite{Ristimaki2017} \\
            Helsinki Region-Travel Time Matrix 2018 & Travel time and distance information for routes between all MetropAccess-YKR-grid cell centroids in the Capital Region of Helsinki & Use on travel time comparison calculations & Digital Geography Lab & \cite{Tenkanen2018} \\
            MetropAccess-YKR-grid & Statistical grid of 250 x 250 meter cells for monitoring urban structure, Helsinki Capital Region area & Use on travel time comparison calculations & Toivonen et al., Statistics Finland & \cite{Toivonen2014a}, \cite{StatisticsFinland2020} \\
            Regional population density 2012 & Population density with municipality boundaries & Visualisation in R & Statistics Finland & \cite{StatisticsFinland2012} \\
            Subdivisions in Helsinki Capital Region & Subdivisions in the municipalities of Helsinki Capital Region & Visualisation in R & Helsingin, Espoon, Vantaan ja Kauniaisten mittausorganisaatiot & \cite{HelsinginEspoonVantaanjaKauniaistenmittausorganisaatiot2011} \\
            \bottomrule
        \end{tabular}}
    \end{table} 
\end{hyphenrules}
...
\begin{hyphenrules}{nohyphenation}
    \begin{table}[H]
        \centering
        \caption[Essential software packages in thesis]{Essential software packages used in the thesis. The complete list is available at the script repositories in GitHub.} 
        \label{tab:used_soft}
        \def\arraystretch{1.2}
        \setlength\tabcolsep{1.2ex}
...
Helsinki Travel-time Matrix 2018 and the survey data of this thesis operate in different spatial units. Travel-time Matrix 2018 uses the MetropAccess-YKR-grid, a spatial dataset based on the Statistics Finland statistics grid with the cell size of 250 x 250 meters. The basic spatial unit of the survey data is the postal code area based on PAAVO open data (\textcolor{red}{lisää lähteet}). Using Python, postal codes were added to each MetropAccess-YKR-grid cell with the logic that the largest area in PAAVO open data (figure~\ref{fig:paavo_ykr}) assigns the postal code in each grid cell. PAAVO postal code areas are not present in all of the cells of MetropAccess-YKR-grid and because of this some cells were assigned a postal code of 99999 to denote missing data (\textcolor{red}{mieti 99999:n käyttö}). As a side product of this postal code assignment, the MetropAccess-YKR-grid was merged with data which tells how much of a cell was contained in the research area (PAAVO open data) and how large was the largest postal code area which dictated the postal code assignment of the current cell.

The data processing script created for this thesis contains detailed features to detect patterns in the survey data. To enhance pattern recognition the records were purged of known false data, namely records and visits made by the author. In addition, one survey respondent reported that they had inserted erroneous data in the survey. These responses were identified and deleted (figure~\ref{fig:gen_workflow}, section 3).

A pandas DataFrame was created to make it easy to view the behaviour of each survey respondent. In addition to this feature, the data processing script writes a text file report about IP addess codes which submitted multiple responses from the same postal code area. The code also identifies whether the duplicate responses have identical values or if they have changed between individual responses. This report of duplicates was then used to determine what to do about the duplicates.

It was decided that if the parking time or walking time value was 60 minutes or greater, that value would be deleted. This value is arbitrary and this research assumes that it is highly unlikely that anybody would generally park 60 minutes away from their final destination which they would reach on foot. An hour of searching for parking is plausible in the center of Helsinki but because it its unlikeliness the same 60 minutes limit was utilised in parktime.

Next the additional spatial data was added to the survey data (figure~\ref{fig:gen_workflow}, section 5). Postal dataframe was added with answer count, parking time mean and walking time mean. Each postal code area also received seven columns to depict the share of classes in percentage. Using Urban Atlas 2012 data, all postal code areas were tested for forest percentage. Using a custom Jenks breaks function (by GitHub user Drewda, \textcolor{red}{add cite}) suitable class breaks were calculated to be used in records.
