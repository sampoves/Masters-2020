...
% note about multirows: it seems that it is not straightforward to vertically center multirows when other cells in the influence of that multirow have wrapped text. Have to hack the vertical positions manually.
\begin{hyphenrules}{nohyphenation}
    \begin{table}[H]
        \centering
        ...
        \scalebox{0.75}
        {\begin{tabular}{ l p{2cm} p{10cm} >{\raggedright\arraybackslash}p{6cm} }
            \toprule
            & Column name, unit & Description & Formula \\
            \midrule
            \multirow{3}{2cm}[-6ex]{timeofday} & r & Rush hour traffic (09:00–11.00, 15.00–17.00) & -- \\
            & m & Midday traffic (09.00–15.00) & -- \\
            & all & An average of all available data. For \textit{TTM}, these are \textit{Rush hour traffic}, \textit{Midday traffic}, and \textit{Route following speed limits without any additional impedances}. For thesis survey data, these are the values of the survey question \code{timeofday}: \textit{Weekday, rush hour}, \textit{Weekday, other than rush hour}, \textit{Weekend}, and \textit{Can't specify, no usual time}. & $\frac{value_1 + value_2 + ... + value_n}{n}$ \\
            \greyrule
            
            \multirow{5}{2cm}[-23ex]{Travel Time Matrix 2018} & sfp, min & Time consumed in searching for parking. In \textit{TTM}, this is 0.42 minutes for all YKR IDs in the entirety of Helsinki Capital Region (\cite{Toivonen2014a}). & -- \\
            ...
            \multirow{4}{2cm}[-14ex]{Thesis data} & sfp\_avg, min & Time consumed, on average, in searching for parking in a postal code area, according to the thesis survey respondents. & $\frac{parktime_1 + parktime_2 + ... + parktime_n}{n}$, while postal code value is constant. \\
            ...
            & pct, \% & How much do the thesis mean values for searching for parking and walking from one's parked car to the final destination constitute of the mean duration of the complete travel chain? & $\frac{thesis\_x\_sfp + thesis\_x\_wtd}{ttm\_x\_drivetime}$ \\
            \greyrule
            
            \multirow{4}{2cm}[-9ex]{Compare TTM and thesis} & sfp, \% & Compare \textit{TTM} and thesis survey values for searching for parking. & $\frac{thesis\_x\_sfp}{ttm\_sfp}$, where $x$ is $r$, $m$, or $all$. \\
